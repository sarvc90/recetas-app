...
function renderMisRecetas(recetas) {
    recetas.forEach(receta => {
        ...
        `;
        card.querySelector(".edit-btn").addEventListener("click", (e) => { e.stopPropagation(); abrirModalEdicion(receta); });
        card.querySelector(".delete-btn").addEventListener("click", (e) => { e.stopPropagation(); eliminarReceta(receta.id, card); });
        card.addEventListener("click", (e) => {
            if (!e.target.classList.contains('edit-btn') && !e.target.classList.contains('delete-btn')) abrirModal(receta);
        });
...
function abrirModalEdicion(receta) {
    ...
    document.getElementById('editRecipeIngredients').value = typeof receta.ingredientes === "string" ? receta.ingredientes : receta.ingredientes.join('\n');
    document.getElementById('editRecipeInstructions').value = receta.instrucciones;
    editImagePreview.innerHTML = receta.imagenUrl ? `<img src="${receta.imagenUrl}" alt="Preview">` : '';
...
editImageInput?.addEventListener('change', (e) => {
    ...
    if (file) {
        ...
        reader.onload = (e) => { editImagePreview.innerHTML = `<img src="${e.target.result}" alt="Preview">`; };
